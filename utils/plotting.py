    ...
    for i in range(4):
      ...
      for j in range(4):
        ...
        if i == j:
          ...
          _, bins, _ = axes[i,j].hist(model.get_numpy(original[:,i]), bins=bin, density=True, histtype='step', color='red')
        ...
        if i < j:
          ...
          axes[i,j].hist2d(model.get_numpy(original[:,i]), model.get_numpy(original[:,j]), bins = [bini, binj], density=True, cmap='Reds')
        
        if i > j:
          bini = get_bins(sampled[:,i])
          binj = get_bins(sampled[:,j])
          axes[i,j].hist2d(model.get_numpy(sampled[:,j]), model.get_numpy(sampled[:,i]), bins = [binj, bini], density=True, cmap="Blues")
