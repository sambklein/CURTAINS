...
def fitPlot(model, data, edges, **kwargs):
  '''
  ...
  #TODO: Save this somewhere better.
  plt.savefig(f'{get_top_dir()}/massrange_{edges[0]}_{edges[1]}_fit.png')
  plt.clf()


def signalMassSampler(masses, size , fitter='conventional', getStatus=False, getFitPlot=False):
  ...
  ARGS:
  1. path : str : The path to the numpy array with the side band mass. #TODO: Not ideal
  ...
  if fitter.casefold() != 'conventional':
    raise NotImplementedError(f"{fitter} does not exist. Only conventional fitting with zfit backend is implemented.")
  if len(masses) <= 0:
    raise ValueError("No masses found to fit. Please pass a valid mass array.")
  
  edge1, edge2 = zfitDataHandler(masses)
  ...
  model = fu.linearPDF(obs=massRange, slope=f_slope, intercept=f_intercept)

  nll = zfit.loss.UnbinnedNLL(model=model, data=data)
  minimizer = zfit.minimize.Minuit()
  result = minimizer.minimize(nll)

  mass_sw =  model.sample(size, limits = (edge1, edge2)).numpy()

  if getFitPlot:
    fitPlot(model, data, (masses.min(), masses.max()) )


  if getStatus:
    return mass_sw, (result, result.hesse())
